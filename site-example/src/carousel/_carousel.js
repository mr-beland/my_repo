import './carousel.css' // импорт css возможен, через любой JS Файл.

export function createCarousel (elemeId, images, options) {
    if (!options) { // проверка - если в параметр options не добавлены данные
        options = {
            loop: 1,
            indicators: 1,
            controls: 1
    ...
    for (let item of images) {
        ...
        // $mainContainer.append($i ndicatorElement); 

        // let itemWidth = $carouselContainer.offsetWidth;
        let itemWidth = $carouselContainer.offsetWidth;
        
        const $indicatorsContainer = document.querySelector('.carousel-indicators');
        // указываем изначальное количество картинок
